...
## TIL 👨‍🏫
**26번째 줄** dist[warp] == -1  
순간이동은 걷는 것보다 빠르니까 어딘가에 도착한다면 무조건 그 지점에 처음 방문할 것이라 생각함.  
그래서 이 조건은 필요없을 거라 생각했는데 cur = 0 -> warp = 0 반복이고 계속 무한루프 돌게 됨.  
이거 방지하려면 한 번 방문한 곳은 방문하지 않도록 위와 같은 조건 추가해야됨 ❗❗
